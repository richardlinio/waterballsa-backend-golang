...
**位置**: [internal/service/auth.go:97-103](../internal/service/auth.go#L97-L103)
...
**問題分析**:
- 登出路由已被 JWT 中介層保護，傳入的 token 不可能為空
- 這個檢查是多餘的，且給使用者錯誤的安全感
- 使用者認為登出後 token 會失效，但實際上 token 仍然有效直到過期

**建議修正方案**:

**方案 A: 實作 Token 黑名單機制**
```go
// 在 service 層新增 token 黑名單介面
type TokenBlacklistRepository interface {
    Add(ctx context.Context, token string, expireAt time.Time) error
    Exists(ctx context.Context, token string) (bool, error)
}

func (s *authService) Logout(ctx context.Context, token string) error {
    // 解析 token 取得過期時間
    expireAt, err := s.tokenGenerator.ParseExpireTime(token)
    if err != nil {
        return apperror.InternalError(err)
    }

    // 將 token 加入黑名單直到其自然過期
    if err := s.blacklistRepo.Add(ctx, token, expireAt); err != nil {
...
// 在 middleware 中檢查黑名單
func (m *authMiddleware) CheckBlacklist(c *gin.Context) {
    token := extractToken(c)
    exists, err := m.blacklistRepo.Exists(c.Request.Context(), token)
    if err != nil {
...
**方案 B: 縮短 Token 有效期限 + 文件說明**
```go
// 如果黑名單機制成本太高，可以：
// 1. 將 access token 有效期限縮短 (例如 5-15 分鐘)
// 2. 在文件中明確說明登出只清除客戶端 cookie
// 3. 移除伺服器端的 Logout 方法，只在客戶端處理

func (h *AuthHandler) Logout(c *gin.Context) {
    // 只清除 cookie，token 會在短時間內自然過期
    h.clearCookie(c)
...
**影響等級**: 🔴 高 - 影響使用者對系統安全性的認知

---

## 中優先級問題

### 2. 雙重 Token 生成來源導致不一致風險

**位置**:
- [internal/infrastructure/auth/token_generator.go](../internal/infrastructure/auth/token_generator.go)
- [internal/infrastructure/auth/jwt.go](../internal/infrastructure/auth/jwt.go)

**問題描述**:
系統中存在兩個 token 生成機制：
1. 自定義的 `TokenGenerator.Generate()` - 用於登入
2. `gin-jwt` 中介層內部生成 - 用於 refresh

**問題分析**:
```go
// TokenGenerator 生成的 claims
...
// gin-jwt 中介層生成的 claims 可能不同
// 這會導致 login token 和 refresh token 結構不一致
```

**建議修正方案**:

**方案 A: 統一使用 gin-jwt 的 token 生成機制**
```go
// 移除自定義 TokenGenerator，使用 gin-jwt 的 PayloadFunc
...
// 在 handler 中使用 gin-jwt 的 LoginHandler
func (h *AuthHandler) Login(c *gin.Context) {
    h.jwtMiddleware.LoginHandler(c)
}
```

**方案 B: 確保兩個生成器使用相同的 claims 結構**
```go
// 定義共用的 claims builder
type ClaimsBuilder struct{}

func (cb *ClaimsBuilder) Build(user *model.User, expireTime time.Time) jwt.MapClaims {
    return jwt.MapClaims{
        "user_id":    user.ID,
        "username":   user.Username,
        "role":       user.Role,
...
func (tg *jwtTokenGenerator) Generate(user *model.User) (string, time.Time, error) {
    ...
    claims := tg.claimsBuilder.Build(user, expireTime)
    // ... 生成 token
}

// gin-jwt PayloadFunc 使用
PayloadFunc: func(data interface{}) jwt.MapClaims {
    if user, ok := data.(*model.User); ok {
        expireTime := time.Now().Add(cfg.AccessTokenTimeout)
        return claimsBuilder.Build(user, expireTime)
    }
    return jwt.MapClaims{}
}
```

**影響等級**: 🟠 中 - 可能導致 token 結構不一致，影響客戶端解析

---

### 3. ExtractIdentity 未提取 Experience 欄位

**位置**: [internal/middleware/auth.go:12-40](../internal/middleware/auth.go#L12-L40)
...
func ExtractIdentity(c *gin.Context) interface{} {
    ...
    // 提取了 user_id, username, role
    // 但沒有提取 experience，儘管 token 中包含此欄位

    return &model.User{
        ID:       int64(userID),
        Username: username,
        Role:     role,
        // ExperiencePoints: ??? 未設定，預設為 0
    }
}
```

**問題分析**:
- Token 生成時包含 `experience` claim
- 但從 token 還原 User 物件時未讀取此欄位
- 如果後續程式碼依賴 context 中的使用者經驗值，會得到錯誤的 0 值
...
func ExtractIdentity(c *gin.Context) interface{} {
    ...
    // 新增：提取 experience 欄位
    experience, ok := claims["experience"].(float64)
    if !ok {
        // 如果沒有 experience claim，記錄警告但不拒絕請求
        // (考慮向後相容性)
        experience = 0
    }
    ...
    return &model.User{
        ...
        Username:        username,
        Role:            role,
        ExperiencePoints: int32(experience),  // 新增此行
    }
}
```

**影響等級**: 🟠 中 - 導致資料不一致，可能影響業務邏輯
